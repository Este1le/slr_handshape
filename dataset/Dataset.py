...
class SignLanguageDataset(torch.utils.data.Dataset):
    ...
    def load_annotations(self):
        ...
        max_sent_length = self.dataset_cfg["max_sent_length"]
        self.annotation = []
        for a in annotations:
            if a['num_frames'] <= max_sent_length:
    ...
    def __getitem__(self, idx):
        return {k:v for k, v in self.annotation[idx].items() \
            if k in [
                'name','gloss','text','num_frames','sign', 'signer', 'alignment',
                'handshape-right', 'handshape-left',
