...
class S3D_backbone(torch.nn.Module):
    def __init__(self, 
        in_channel=3,
        use_block=5, freeze_block=0, stride=2, pretrained_ckpt='scratch'):
    ...
    def load_s3d_model_weight(self, model_path):
        if 'actioncls' in model_path:
            ...
            new_dict = {}
            for k,v in state_dict.items():
                k = k.replace('recognition_network.visual_backbone_keypoint.', '')
                new_dict[k] = v
            state_dict = new_dict
            try:
            ...
            try:
                ...
                neq_load_customized(self, new_dict, verbose=True)  
        else:
            raise ValueError  
    ...
    def forward(self, sgn_videos, sgn_lengths=None):
        ...
        valid_len_out = torch.floor(sgn_lengths*T_out/T_in+0.001).long() #B,
        for bi in range(B):
            sgn_mask[bi, :, :valid_len_out[bi]] = True
        sgn_mask_lst.append(sgn_mask)
        valid_len_out_lst.append(valid_len_out)
    
        return {'sgn_feature':fea_lst[-1], 'sgn_mask':sgn_mask_lst[-1], 
