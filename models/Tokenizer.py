...
class BaseHandshapeTokenizer(BaseTokenizer):
    def __init__(self, tokenizer_cfg, hand):
        super().__init__(tokenizer_cfg)
        with open(tokenizer_cfg['handshape2id_file_'+hand],'rb') as f:
            self.hs2id = pickle.load(f) #
        self.hs2id = defaultdict(lambda: self.hs2id['<unk>'], self.hs2id)
        self.id2hs = {}
        for hs, id_ in self.hs2id.items():
            self.id2hs[id_] = hs     
        self.lower_case = tokenizer_cfg.get('lower_case',True)
    ...
    def convert_tokens_to_ids(self, tokens):
        if type(tokens)==list:
            ...
            return self.hs2id[tokens]

    def convert_ids_to_tokens(self, ids):
        if type(ids)==list:
            return [self.convert_ids_to_tokens(i) for i in ids]
        else:
            return self.id2hs[ids]
    
    def __len__(self):
        return len(self.id2hs)
...
class HandshapeTokenizer_S2G(BaseHandshapeTokenizer):
    def __init__(self, tokenizer_cfg, hand):
        super().__init__(tokenizer_cfg, hand)
        self.hand = hand
        if '<s>' in self.hs2id:
            self.silence_token = '<s>'
            self.silence_id = self.convert_tokens_to_ids(self.silence_token)
        elif '<si>' in self.hs2id:
            self.silence_token = '<si>'
            self.silence_id = self.convert_tokens_to_ids(self.silence_token)
        else:
            raise ValueError            
    ...
    def __call__(self, batch_hs_seq):
        if type(batch_hs_seq[0][0]) != list:
            new_batch_hs_seq = []
            for ii, hs_seq in enumerate(batch_hs_seq):
                hs_seq = hs_seq.split()
                hs_seq = [[h] for h in hs_seq]
                new_batch_hs_seq.append(hs_seq)
            batch_hs_seq = new_batch_hs_seq
        max_length = max([len(sum(hs_seq, [])) for hs_seq in batch_hs_seq])
        hs_lengths, batch_hs_ids, seg_ids = [], [], []
        for ii, hs_seq in enumerate(batch_hs_seq):
            if hs_seq == None:
                return {}
            # seg = [0]
            # for hss in hs_seq:
            #     seg.append(len(hss)+seg[-1])
            # seg_ids.append(seg)
            hs_ids = []
            segs = []
            for hss in hs_seq:
                new_hss = []
                for hs in hss:
                    hs = self.hs2id[hs.lower() if self.lower_case else hs]
                    if new_hss == [] or (new_hss != [] and new_hss[-1]!=hs):
                        new_hss.append(hs)
                segs.append(new_hss)
                hs_ids.extend(new_hss)
                #segs.append([self.hs2id[hs.lower() if self.lower_case else hs] for hs in hss])
            seg_ids.append(segs)
            hs_lengths.append(len(hs_ids))
            hs_ids = hs_ids+(max_length-len(hs_ids))*[self.pad_id]
            batch_hs_ids.append(hs_ids)
        hs_lengths = torch.tensor(hs_lengths)
        batch_hs_ids = torch.tensor(batch_hs_ids)
        return {'hs_lengths_'+self.hand:hs_lengths, 'handshape_labels_'+self.hand: batch_hs_ids, 'hs_seg_ids_'+self.hand: seg_ids}
